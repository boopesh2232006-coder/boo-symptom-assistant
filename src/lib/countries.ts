...
export const parsePhoneNumber = (fullNumber: string, countries: Country[]): { country: Country; localNumber: string } => {
  const cleaned = (fullNumber || "").trim();
  if (cleaned.startsWith("+")) {
    // Sort countries by dialCode length descending so we check longer dial codes first (like +1-242 before +1)
    const sortedCountries = [...countries].sort((a, b) => b.dialCode.length - a.dialCode.length);
    for (const country of sortedCountries) {
      if (cleaned.startsWith(country.dialCode)) {
        const local = cleaned.slice(country.dialCode.length).trim();
        return { country, localNumber: local };
      }
    }
  }
  // Default to US or first country if no match found
